...
class IMDb {
 public:
	...
	// structuri adaugate pentru implementarea temei
	std::unordered_map<std::string, class Movie> Movies;
	std::unordered_map<std::string, class Actor> Actors;
	std::unordered_map<std::string, class User> Users;
	std::unordered_map<std::string, class Director> Directors;
	std::unordered_map<std::string, struct Pair> Pairs;
	int longest_career;
	std::string longest_career_act;
	int most_influential;
	std::string most_influential_dir;
	std::vector <struct Popular> Popular;
	int verify;
